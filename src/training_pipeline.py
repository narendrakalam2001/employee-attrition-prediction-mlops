# ============================================================
# TRAINING PIPELINE — Employee Attrition ML System
# ============================================================
...
def run_training():
    ...
    # ─────────────────────────────────────────────────────────
    DATA_PATH = r"D:\Data Science Datasets\Employee Attrition Prediction\WA_Fn-UseC_-HR-Employee-Attrition.csv"
    ...
    logger.info("Dataset  |  shape=%s  |  attrition_rate=%.3f", df.shape, y.mean())
    ...
    # ─────────────────────────────────────────────────────────
    # 13. ATTRITION ENGINE DECISIONS
    # ─────────────────────────────────────────────────────────
    decisions = attrition_engine(X_test, y_prob_sel, thr_sel)
    decision_counts = pd.Series(decisions).value_counts()

    print("\nATTRITION ENGINE DECISIONS")
    ...
    try:
        ...
        rep_scores = cross_val_score(
            ...
            scoring = "f1",
            cv      = rskf,
            n_jobs  = 1      # n_jobs=1 avoids joblib worker instability on heavy CV
        )
    ...
    final_decisions = attrition_engine(X_test, final_probs, thr_cal)
